...
class _SignUpPageState extends State<SignUpPage> {
  ...
  // Added a dedicated Notifier for the entire form's validity
  final ValueNotifier<bool> isFormValid = ValueNotifier(false);

  @override
  void initState() {
    super.initState();
    // Attach listeners to update validation on every single keystroke
    emailController.addListener(_checkValidation);
    passwordController.addListener(_checkValidation);
    fullNameController.addListener(_checkValidation);
    validateEmail.addListener(_checkValidation);
    validatePassword.addListener(_checkValidation);
    rememberPassword.addListener(_checkValidation);
  }

  @override
  void dispose() {
    emailController.removeListener(_checkValidation);
    passwordController.removeListener(_checkValidation);
    fullNameController.removeListener(_checkValidation);
    validateEmail.removeListener(_checkValidation);
    validatePassword.removeListener(_checkValidation);
    rememberPassword.removeListener(_checkValidation);
  ...
  void _checkValidation() {
    // This safely combines both text checks and the custom package logic validation flags
    bool emailValid = validateEmail.value || emailController.text.contains('@');
    bool passwordValid = validatePassword.value || passwordController.text.length >= 6;

    isFormValid.value = passwordValid &&
        emailValid &&
        rememberPassword.value &&
        fullNameController.text.isNotEmpty;
  }

  // Intercepts the Google Sign-Up call and forwards to Step 2 with Google account credentials
  Future<void> _handleGoogleSignUp(BuildContext context) async {
    try {
      FirebaseAuthCubit authCubit = FirebaseAuthCubit.get(context);
      
      // Triggers the authentication flow pop-up window
      await authCubit.signInWithGooglePressed();
      
      final currentState = authCubit.state;
      
      if (currentState is CubitAuthConfirmed) {
        // Since Google already verified them, send them right to Step 2 
        // to pick their unique Skrim Chat username!
        Get.to(
          UserNamePage(
            emailController: TextEditingController(text: currentState.user.email),
            passwordController: TextEditingController(text: ""), // Google handles auth, no password needed
            fullNameController: TextEditingController(text: currentState.user.displayName ?? ""),
          ),
          duration: const Duration(seconds: 0),
        );
      }
    } catch (e) {
      ToastShow.toast("Google Sign Up Interrupted: ${e.toString()}");
    }
  }
  ...
  Widget customTextButton() {
    return ValueListenableBuilder<bool>(
      ...
      builder: (context, bool formIsValid, child) {
        return Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            // Your original Email/Password "Next" action button
            CustomElevatedButton(
              isItDone: true,
              isThatSignIn: true,
              nameOfButton: StringsManager.next.tr,
              blueColor: formIsValid, // Turns bright blue when true
              onPressed: () async {
                if (formIsValid) {
                  Get.to(
                    UserNamePage(
                  ...
                  // Detailed fallback debug message so you know exactly which logic flag is failing
                  String errorLog = "Missing: ";
                  if (fullNameController.text.isEmpty) errorLog += " [Name] ";
                  if (emailController.text.isEmpty) errorLog += " [Email] ";
                  if (passwordController.text.isEmpty) errorLog += " [Password] ";
                  if (!rememberPassword.value) errorLog += " [Checkbox] ";
                  
                  ToastShow.toast("Form Invalid! $errorLog");
                }
              },
            ),
            ...
            onPressed: () async {
              ...
              if (validateEdits) {
                ...
                if (widget.passwordController.text.isEmpty) {
                  ...
                  if (currentUser != null) {
                    addNewUser(
                      CubitAuthConfirmed(currentUser),
                      userCubit,
                    );
                    moveToMain(CubitAuthConfirmed(currentUser));
                  }
                } else {
