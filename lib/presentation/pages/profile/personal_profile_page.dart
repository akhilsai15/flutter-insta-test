...
class _ProfilePageState extends State<PersonalProfilePage> {
  ...
  List<Widget> widgetsAboveTapBarsForWeb(UserPersonalInfo userInfo) {
    return [
      ...
      editProfileButtonForWeb(userInfo),
      const SizedBox(width: 10),
      GestureDetector(
        onTap: () async => bottomSheet(createNewData: false), // ← settings opens the menu sheet
        child: const Icon(Icons.settings_rounded, color: ColorManager.black),
      ),
    ];
  }

  Widget editProfileButtonForWeb(UserPersonalInfo userInfo) {
    return GestureDetector(
      onTap: () async {
        await Go(context).push(page: EditProfilePage(userInfo)); // ← navigates to edit page
        rebuildUserInfo.value = true;
      },
      child: Container(
