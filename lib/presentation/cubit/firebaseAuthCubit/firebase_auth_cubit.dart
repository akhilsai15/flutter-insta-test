...
import '../../../data/data_sources/remote/firebase_auth.dart'; // ADD THIS IMPORT
...
class FirebaseAuthCubit extends Cubit<FirebaseAuthCubitState> {
  ...
  Future<void> signInWithGooglePressed() async {
    emit(CubitAuthConfirming()); // Puts UI into loading state safely
    try {
      // Direct data source call for testing, bypassing the missing repository reference
      final userCredential = await FirebaseAuthentication.signInWithGoogle();
      if (userCredential.user != null) {
        emit(CubitAuthConfirmed(userCredential.user!));
      } else {
        emit(CubitAuthFailed("User is null"));
      }
    } catch (e) {
      emit(CubitAuthFailed(e.toString()));
    }
