...
class _WebScreenLayoutState extends State<WebScreenLayout> {
  ...
  void onSelectedProfileMenu(int item) {
    if (item == 0) navigationTapped(5); // Profile

    if (item == 1) {
      // Settings → navigate to profile page and open the settings bottom sheet
      navigationTapped(5);
        ...
        if (mounted) {
          showModalBottomSheet(
            ...
            builder: (BuildContext ctx) => BlocProvider.value(
              value: BlocProvider.of<FirebaseAuthCubit>(context),
              child: _SettingsBottomSheet(personalId: myPersonalId),
            ),
          );
        }
      });
...
class _SettingsBottomSheet extends StatelessWidget {
  ...
  const _SettingsBottomSheet({required this.personalId});
      ...
      child: Column(
        ...
        children: [
          ...
          GestureDetector(
            ...
            child: _settingsRow(Icons.language_rounded, "Change Language"),
          ),
          ...
          GestureDetector(
            ...
            child: _settingsRow(Icons.brightness_4_outlined, "Change Theme"),
          ),
          const Divider(indent: 40, endIndent: 15, color: ColorManager.grey),
            ...
            builder: (context, state) {
              return GestureDetector(
                onTap: () async {
                  ...
                  FirebaseAuthCubit authCubit = FirebaseAuthCubit.get(context);
                  await authCubit.signOut(userId: personalId);
                  await sharePrefs.clear();
                  Get.offAll(const LoginPage(),
                      duration: const Duration(milliseconds: 0));
                },
                child: _settingsRow(Icons.logout_rounded, "Log Out"),
              );
            },
          ),
  ...
  Widget _settingsRow(IconData icon, String label) {
    return SizedBox(
      ...
      child: Row(
        children: [
          Icon(icon, color: ColorManager.black),
          const SizedBox(width: 15),
          Text(label,
              style: getNormalStyle(color: ColorManager.black, fontSize: 15)),
        ],
      ),
    );
  }
